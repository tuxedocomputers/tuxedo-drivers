...
static const struct dmi_system_id lwl_nb05_ec_id_table[] __initconst = {
	{
		.ident = "LWL Pulse 14 Gen3",
		.matches = {
...
MODULE_DEVICE_TABLE(dmi, lwl_nb05_ec_id_table);

static int __init lwl_nb05_ec_init(void)
{
	if (!dmi_check_system(lwl_nb05_ec_id_table))
		return -ENODEV;
